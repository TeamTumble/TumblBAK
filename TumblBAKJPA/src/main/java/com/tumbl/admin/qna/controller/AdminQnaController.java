...
public class AdminQnaController {
	...
	*//**************************************************************
	...
	public String qnaDetail(@ModelAttribute QnaVO pvo, Model model) {
		...
		logger.info("q_num = " + pvo.getQnum());
		QnaVO detail = new QnaVO();
		detail = adminQnaService.qnaDetail(pvo);
	...
	*//**************************************************************
	...
	public String qnaInsert(@ModelAttribute QnaVO qvo, Model model, HttpServletRequest request)
		...
		result = adminQnaService.qnaInsert(qvo);
		if (result == 1) {
			url = "/admin/qna/qnaList.do";
		} else {
			model.addAttribute("code", 1);
			url = "/admin/qna/writeForm.do";
		}
		return "redirect:" + url;
	}
	...
	*//**************************************************************
	 ...
	 **************************************************************//*
	@ResponseBody
	@RequestMapping(value = "/qna/pwdConfirm.do", method = RequestMethod.POST, produces = "text/plain; charset=UTF-8")
	public String pwdConfirm(@ModelAttribute QnaVO qvo) {
		logger.info("pwdConfirm 호출 성공");
		String value = "";
		// 아래 변수에는 입력 성공에 대한 상태값 저장(1 or 0)
		int result = adminQnaService.pwdConfirm(qvo);
		if (result == 1) {
			value = "성공";
		} else {
			value = "실패";
	...
	*//**************************************************************
	...
	public String qnaUpdate(@ModelAttribute QnaVO qvo, HttpServletRequest request) throws IllegalStateException, IOException {
		...
		if (result == 1) {
			...
			url = "/qna/qnaDetail.do?q_num=" + qvo.getQnum();
		} else {
			url = "/qna/updateForm.do??q_num=" + qvo.getQnum();
		}
		return "redirect:" + url;
	}
	...
	*//**************************************************************
	...
	public String qnaDelete(@ModelAttribute QnaVO qvo, HttpServletRequest request) throws IOException {
		...
		result = adminQnaService.qnaDelete(qvo.getQnum());
		if (result == 1) {
			url = "admin/qna/qnaList.do";
		} else {
			url = "admin/qna/qnaDetail.do?q_num=" + qvo.getQnum();
		}
		return "redirect:" + url;
	}
}
