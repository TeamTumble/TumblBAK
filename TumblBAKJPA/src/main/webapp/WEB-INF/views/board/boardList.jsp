...
<body>
	<div class="contentContainer">
		...
		<form name="detailForm" id="detailForm">
			<input type="hidden" name="qnum" id="qnum"> <input
				...
				<thead>
					<tr>
						<th data-value="qnum" class="order">글번호 <c:choose>
								<c:when test="${data.order_by=='qnum' and data.order_sc=='ASC'}">▲</c:when>
								<c:when
					...
					<c:choose>
						<c:when test="${not empty boardList}">
							<c:forEach var="board" items="${boardList}"
								varStatus="status">
								<tr class="tac" data-num="${board.qnum}">
									<td>${board.qnum}</td>
	...
	<div class="contentContainer">
		...
		<div id="boardList">
			<table summary="게시판 리스트">
				...
				<thead style="background-color: #8ff187;">
					<tr>
						<th data-value="qnum" class="order">글번호 <c:choose>
								<c:when
									test="${data.order_by=='qnum'
